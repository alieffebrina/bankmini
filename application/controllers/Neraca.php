...
class Neraca extends CI_Controller
{
    ...
    public function index(){
		...
		$data['COAD'] = $this->db->get_where('tb_mastercoa', ['neraca' => '1', 'grupcoa'=>'Aktiva'])->result_array();
		$data['COAK'] = $this->db->get_where('tb_mastercoa', ['neraca' => '1', 'grupcoa'=>'Hutang','grupcoa'=>'Modal' ])->result_array();
		$data['tglawal'] = $this->db->query('select MIN(tgl_update) as tglawal from tb_jurnal')->result_array();
        // $data['kk'] = $this->M_KasKeluar->getAll();
		$this->load->view('template/header');
		$this->load->view('template/sidebar', $data);
		$this->load->view('v_neraca/v_neraca', $data);
	...
	public function sort(){
		...
		$this->load->view('template/header');
		$this->load->view('template/sidebar', $data);
		$this->load->view('v_neraca/v_neracasort', $data);
		$this->load->view('template/footer');
	...
	public function excel($tglawal, $tglakhir){
		...
		echo $tglawal;
        // $this->load->view('v_neraca/excelneraca', $data);
	}

	public function pdf($tglawal, $tglakhir){
		$COAD = $this->db->get_where('tb_mastercoa', ['neraca' => '1', 'grupcoa'=>'Aktiva'])->result_array();
		$COAK = $this->db->get_where('tb_mastercoa', ['neraca' => '1', 'grupcoa'=>'Hutang','grupcoa'=>'Modal' ])->result_array();
        $pdf = new FPDF('L','mm',array('148', '210'));
        // membuat halaman baru
        $pdf->AddPage();  
        $pdf->SetFont('Arial','',8,'C');
        $pdf->Cell(110,5,'LAPORAN NERACA',0,1,'L');
        ...
        $pdf->Cell(10,2,'',0,1);
    	$pdf->Cell(100,5, 'NO AKUN', 1,0);
        $pdf->Cell(40,5,'DEBET',1,0);
        $pdf->Cell(40,5,'KREDIT',1,1);
        foreach ($COAD as $COAD ) {
	    	...
	    	$pdf->Cell(100,5, $COAD['kode_coa'].' - '.$COAD['akun'], 1,0);
	        $pdf->Cell(40,5,'Rp. '.number_format($nominal),1,0);
	        $pdf->Cell(40,5,'Rp. 0',1,1);
        }
        foreach ($COAK as $COAK ) {
        	$this->db->select_sum('nominal_debet');
	        $this->db->select_sum('nominal_kredit');
	        $this->db->join('tb_jurnal', 'tb_jurnal.id_jurnal = tb_detailjurnal.id_jurnal');
	    	...
	    	$pdf->Cell(100,5, $COAK['kode_coa'].' - '.$COAK['akun'], 1,0);
	        $pdf->Cell(40,5,'Rp. '.number_format($nominal),1,0);
	        $pdf->Cell(40,5,'Rp. 0',1,1);
