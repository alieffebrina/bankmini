...
class Modal extends CI_Controller
{
    ...
    public function index(){
		...
		$data['activeMenu'] = $this->db->get_where('tb_submenu', ['submenu' => 'Perubahan Modal'])->row()->id_menus;
		$data['COAD'] = $this->db->get_where('tb_mastercoa', ['perubahan_modal' => '1'])->result_array();
		$data['COALR'] = $this->db->get_where('tb_mastercoa', ['laba_rugi' => '1'])->result_array();
        // $data['kk'] = $this->M_KasKeluar->getAll();
		$this->load->view('template/header');
		$this->load->view('template/sidebar', $data);
		$this->load->view('v_modal/v_modal', $data);
	...
	public function sort(){
		...
		$this->load->view('template/header');
		$this->load->view('template/sidebar', $data);
		$this->load->view('v_modal/v_modalsort', $data);
		$this->load->view('template/footer');
	...
	public function excel($tglawal, $tglakhir){
        ...
        $this->load->view('v_modal/excelmodal', $data);
	}

	public function pdf($tglawal, $tglakhir){
		$COAD = $this->db->get_where('tb_mastercoa', ['perubahan_modal' => '1'])->result_array();
		$COALR = $this->db->get_where('tb_mastercoa', ['laba_rugi' => '1'])->result_array();
       
       	$pdf = new FPDF('L','mm',array('148', '210'));
        // membuat halaman baru
        $pdf->AddPage();  
        $pdf->SetFont('Arial','',8,'C');
        $pdf->Cell(110,5,'LAPORAN PERUBAHAN MODAL',0,1,'L');
    	...
    	$pdf->Cell(140,5, 'Laba / Rugi', 1,0);
        $pdf->Cell(40,5,'Rp. '.number_format($lr),1,1);
        $pdf->Cell(10,2,'',0,1);
        $saatini = $modal + $lr; 
    	$pdf->Cell(140,5, 'Modal Saat Ini', 1,0);
        $pdf->Cell(40,5,'Rp. '.number_format($saatini),1,1);
