...
<script>
	...
	$('.kategori').change(function() {		
		if (this.value != 'salah') {
			...
			$.get(baseUrl + 'mtransaksi/detailTransaksi/' + this.value, function(result) {
				...
				$('#keterangan').val(data.kategori+" - "+data.deskripsi)
				...
				$.get(baseUrl + 'transaksi/getTransaksi', function(result) {
					...
					if (data.length != 0) {
						...
						data.forEach(function(res) {
							...
							d = d.split(' ')[0]
							$('.transaksiField').append('<option tipe="transaksi" keterangan="' + res.keterangan + '" type="' + res.type + '" jenis="'+jenis+'" kredit="' + res.kredit + '" debet="' + res.debet + '" nominal="' + res.nominal + '" value="' + res.id_transaksi +'">' + d + ' ' + res.namaTransaksi + '(' + res.noIden + ') <b>' + res.keterangan + '</b></option>')
						})
					} else {
						$('.transaksiField').html('')
				...
				$.get(baseUrl + 'kaskeluar/getKasKeluar', function(result) {
					...
					if (data.length != 0) {
						...
						data.forEach(function(res) {
							...
							d = d.split(' ')[0]
							$('.transaksiField').append('<option tipe="kk" jenis="kredit" keterangan="' + res.keterangan + '" nominal="' + res.nominal + '" value="' + res.id_kk + '">' + d + ' ' + res.keterangan + ' (' + res.kode_kas_keluar + ') <b>' + formatRupiah(res.nominal, "Rp. ") + '</b></option>')
						})
					} else {
						$('.transaksiField').html('')
						$('.transaksiField').attr('disabled', 'disabled')
				...
				$.get(baseUrl + 'kasmasuk/getKasMasuk', function(result) {
					...
					if (data.length != 0) {
						...
						data.forEach(function(res) {
							var d = res.tgltransaksi;
							d = d.split(' ')[0]
							$('.transaksiField').append('<option tipe="km" jenis="debet" keterangan="' + res.keterangan + '" nominal="' + res.nominal + '" value="' + res.id_km + '">' + d + ' ' + res.keterangan + ' (' + res.kode_kas_masuk + ') <b>' + formatRupiah(res.nominal, "Rp. ") + '</b></option>')
						})
					} else {
						$('.transaksiField').html('')
						$('.transaksiField').attr('disabled', 'disabled')
	...
	$('.transaksiField').change(function() {
		...
		let jenis = $('.transaksiField option:selected').attr('jenis')
		if (tipe == 'kk') {
